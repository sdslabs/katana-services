...
def generate_flag():
    ...
    flag = (
        "katana{"
        + "".join(random.choice(flag_chars) for _ in range(length_of_flag))
        + "}"
    )
    return flag
...
def update_flag(mongo_client, team_name, challenge_name):
    ...
    query = {"username": team_name, "challenges.challengename": challenge_name}

    update = {"$set": {"challenges.$.flag": flag}}
...
def deduct_downtime_scores(mongo_client):
    ...
    for team in challenge_statuses:
        for challenge in challenge_statuses[team]:
            query = {"username": team, "challenges.challengename": challenge}

            update = {
                "$inc": {
                    "score": -(
                        (1 - challenge_statuses[team][challenge]) * down_deduction
                    )
                }
            }
            mongo_db = mongo_client["katana"]
...
def watch_statefulset():
    ...
    for event in w.stream(
        api_instance.list_namespaced_stateful_set, namespace=namespace
    ):
        if event["object"].metadata.name == statefulset_name:
            ...
            if annotations and annotations.get("tick") == "true":
                ...
                updated_statefulset = api_instance.patch_namespaced_stateful_set(
...
def pod_executor(file_path, real_flag, pod_name, pod_namespace, is_getter):
    ...
    resp = stream(
        api_new.connect_get_namespaced_pod_exec,
        pod_name,
        pod_namespace,
        command="/bin/sh",
        stderr=True,
...
def get_exact_name(challenge_name, namespace):
    api = client.CoreV1Api()
    data_json = api.list_namespaced_pod(
        namespace, label_selector="app=" + challenge_name
    )
    if len(data_json.items) == 0:
        print(
            f"Error: No pod for challenge {challenge_name} exists in namespace: {namespace}"
        )
        return None
    else:
...
def successful_submission(submitter_team, submitted_team, mongo_collection, challenge):
    ...
    mongo_collection.update_one(
        {"username": submitter_team}, {"$inc": {"score": challenge["points"]}}
    )
                    ...
                    if true_flag == flag:
                        if team["username"] == team_name:
                            ...
                            if team_name in submissions:
                                if (submitted_team, challenge_name) in submissions[
                                    team_name
                                ]:
